...
//        player.updatePoster(for: Library.shared.movies[0])
        ...
        if windowScene.traitCollection.userInterfaceIdiom == .phone {
            ...
            let detail = MovieDetailViewController(player: player, list: list)
            rootViewController = UINavigationController(rootViewController: detail)
        } else {
            ...
            svc.displayModeButtonVisibility = .always
