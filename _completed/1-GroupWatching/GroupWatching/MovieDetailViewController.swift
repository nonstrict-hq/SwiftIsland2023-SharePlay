...
class MovieDetailViewController: UIViewController {
    ...
    @objc private func presentShareSheet(_ sender: UIBarButtonItem) {
        let activity = CoordinationManager.shared.enqueuedMovie.map { MovieWatchingActivity(movie: $0) }
        ...
        let vc = UIActivityViewController(activityItemsConfiguration: configuration)
    ...
    @objc private func presentSharePlay() {
        let activity = CoordinationManager.shared.groupSession?.activity
            ?? CoordinationManager.shared.enqueuedMovie.map { MovieWatchingActivity(movie: $0) }
        guard let activity else { return }

        do {
            let vc = try GroupActivitySharingController(activity)
            self.present(vc, animated: true)
        } catch {
            print(error)
